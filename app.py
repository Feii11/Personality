...
def prepare_and_compute(upload):
    ...
    # compute G-index aggregated at SIC2 as well (needed by the app unpack)
...
if upload is not None:
    ...
    def _wrapped_plot_radar(*args, **kwargs):
        ...
        def _enforce_min_height_and_margins(*a, **kw):
            ...
            # ensure enough margins so axis labels don't get clipped
        ...
        # apply extra polar/legend tweaks to avoid label overlap / clipping
        try:
            fig.update_layout(
                polar=dict(
    ...
    with tab1:
        ...
        # G-Pairs (pairwise G for trait pairs) — compute and show as bar chart
        try:
    ...
    with tab2:
        ...
        with sim_col_pear2:
            ...
            if pear_viz_choice2 == "Dendrogram":
                fig_pear2 = plot_pearson_dendrogram(pearson2, id_col='SIC_2digit')
            else:
        ...
        # Reuse the top "Filter by SIC 1 Digit" dropdown (sic1_filter) so there's only one dropdown in the UI.
        selected_sic1_g = sic1_filter

        if selected_sic1_g == 'All':
            # Prompt user to pick a specific SIC1 from the top dropdown to view group-level G-Index
            st.info("Select a specific SIC 1 Digit using the 'Filter by SIC 1 Digit' dropdown above to view G-Index per group.")
            # create an empty dataframe with expected columns so downstream logic handles the "no data" case
            ...
            st.markdown(f'<p class="caption-text">✓ Processing {num_sic2} SIC in 2 Digit categories</p>', unsafe_allow_html=True)
